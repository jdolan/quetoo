...
static void Ai_GetUserInfo(const g_entity_t *self, char *userinfo) {
	...
	if (ai_name_suffix == 0) {
		SetUserInfo(userinfo, "name", strcat(ai_name_prefix->string, ai_names[ai_name_index]));
	} else {
		SetUserInfo(userinfo, "name", va("%s %i",
			strcat(ai_name_prefix->string, ai_names[ai_name_index]), ai_name_suffix + 1));
	}
