...
bool R_Rtx_BridgeUpload(r_rtx_bridge_t *bridge, const r_rtx_output_t *output) {
  ...
  SDL_GPUTransferBuffer *transfer = $(r_context.device, createTransferBuffer, &(SDL_GPUTransferBufferCreateInfo) { .usage = SDL_GPU_TRANSFERBUFFERUSAGE_UPLOAD, .size = size });
  void *pixels = $(r_context.device, mapTransferBuffer, transfer, false);
  memcpy(pixels, output->pixels, size);
  $(r_context.device, unmapTransferBuffer, transfer);
  CopyPass *copy = $(r_context.device->commands, beginCopyPass);
  $(copy, uploadTexture, &(SDL_GPUTextureTransferInfo) { .transfer_buffer = transfer, .pixels_per_row = output->extent.width, .rows_per_layer = output->extent.height }, &(SDL_GPUTextureRegion) { .texture = bridge->texture->texture, .w = output->extent.width, .h = output->extent.height, .d = 1 }, true);
  release(copy);
  $(r_context.device, releaseTransferBuffer, transfer);
  return true;
}
