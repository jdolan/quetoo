...
/*
 * @brief Enumeration helper for Cl_WriteConfiguration.
 */
static void Cl_WriteConfiguration_Cvar_enumerate(cvar_t *var, void *data) {
...
static void Cl_WriteConfiguration(void) {
	...
	Cl_WriteBindings(f);
	Cvar_Enumerate(Cl_WriteConfiguration_Cvar_enumerate, (void *) f);
	Fs_Close(f);
}
