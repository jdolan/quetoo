...
static void Cg_Init(void) {
  ...
  cg_addAtmospheric = cgi.AddCvar("cg_add_atmospheric", "1", CVAR_ARCHIVE, "Controls the intensity of atmospheric effects.");
  cg_addDecals = cgi.AddCvar("cg_add_decals", "1", CVAR_ARCHIVE, "Controls decals (bullet holes, blood, etc.).");
  cg_addEntities = cgi.AddCvar("cg_add_entities", "1", 0, "Toggles adding entities to the scene.");
  cg_addFlares = cgi.AddCvar("cg_add_flares", "1", CVAR_ARCHIVE, "Toggles adding flare effects to light sources.");
  cg_addLights = cgi.AddCvar("cg_add_lights", "1", 0, "Toggles adding dynamic lights to the scene.");
  cg_addSprites = cgi.AddCvar("cg_add_sprites", "1", 0, "Toggles adding sprites to the scene.");
  cg_addWeather = cgi.AddCvar("cg_add_weather", "1", CVAR_ARCHIVE, "Controls the intensity of weather effects.");
  cg_bob = cgi.AddCvar("cg_bob", "1", CVAR_ARCHIVE, "Controls weapon bobbing effect.");
  cg_drawBlend = cgi.AddCvar("cg_draw_blend", "1", CVAR_ARCHIVE, "Controls the intensity of screen alpha-blending.");
  cg_drawBlendDamage = cgi.AddCvar("cg_draw_blend_damage", "1", CVAR_ARCHIVE, "Controls the intensity of the blend flash effect when taking damage.");
  cg_drawBlendLiquid = cgi.AddCvar("cg_draw_blend_liquid", "1", CVAR_ARCHIVE, "Controls the intensity of the blend effect while in a liquid.");
  cg_drawBlendPickup = cgi.AddCvar("cg_draw_blend_pickup", "1", CVAR_ARCHIVE, "Controls the intensity of the blend flash effect when picking up items.");
  cg_drawBlendPowerup = cgi.AddCvar("cg_draw_blend_powerup", "1", CVAR_ARCHIVE, "Controls the intensity of the blend flash effect when holding a powerup.");
  cg_drawCrosshair = cgi.AddCvar("cg_draw_crosshair", "1", CVAR_ARCHIVE, "Which crosshair image to use, 0 disables (Default is 1)");
  cg_drawCrosshairAlpha = cgi.AddCvar("cg_draw_crosshair_alpha", "1.0", CVAR_ARCHIVE, "Opacity of the crosshair");
  cg_drawCrosshairColor = cgi.AddCvar("cg_draw_crosshair_color", "default", CVAR_ARCHIVE, "Specifies your crosshair color, in the hex format \"rrggbb\". \"default\" uses white.");
  cg_drawCrosshairHealth = cgi.AddCvar("cg_draw_crosshair_health", "0", CVAR_ARCHIVE, "Method of coloring the crosshair by health. Range from 1-5, 0 disables.");
  cg_drawCrosshairPulse = cgi.AddCvar("cg_draw_crosshair_pulse", "1", CVAR_ARCHIVE, "Pulse the crosshair when picking up items");
  cg_drawCrosshairScale = cgi.AddCvar("cg_draw_crosshair_scale", "1", CVAR_ARCHIVE, "Controls the crosshair scale (size)");
  cg_drawDiagnostics = cgi.AddCvar("cg_draw_diagnostics", "0", CVAR_ARCHIVE, "Draw the client, renderer and sound counters on the HUD");
  cg_drawFps = cgi.AddCvar("cg_draw_fps", "1", CVAR_ARCHIVE, "Draw the frame rate on the HUD");
  cg_drawHud = cgi.AddCvar("cg_draw_hud", "1", CVAR_ARCHIVE, "Render the Heads-Up-Display");
  cg_drawPing = cgi.AddCvar("cg_draw_ping", "1", CVAR_ARCHIVE, "Draw the round trip time to the server on the HUD");
  cg_drawPingWarn = cgi.AddCvar("cg_draw_ping_warn", "200", CVAR_ARCHIVE, "The round trip time, in milliseconds, above which the ping is drawn as lagging");
  ...
  cg_drawTargetName = cgi.AddCvar("cg_draw_target_name", "1", CVAR_ARCHIVE, "Draw the target's name");
  cg_drawWeapon = cgi.AddCvar("cg_draw_weapon", "1", CVAR_ARCHIVE, "Toggle drawing of the weapon model.");
  cg_drawWeaponAlpha = cgi.AddCvar("cg_draw_weapon_alpha", "1", CVAR_ARCHIVE, "The alpha transparency for drawing the weapon model.");
  cg_drawWeaponBob = cgi.AddCvar("cg_draw_weapon_bob", "1", CVAR_ARCHIVE, "If the weapon model bobs while moving.");
  cg_drawWeaponX = cgi.AddCvar("cg_draw_weapon_x", "0", CVAR_ARCHIVE, "The x offset for drawing the weapon model.");
  cg_drawWeaponY = cgi.AddCvar("cg_draw_weapon_y", "0", CVAR_ARCHIVE, "The y offset for drawing the weapon model.");
  cg_drawWeaponZ = cgi.AddCvar("cg_draw_weapon_z", "0", CVAR_ARCHIVE, "The z offset for drawing the weapon model.");
  cg_drawVitalsPulse = cgi.AddCvar("cg_draw_vitals_pulse", "1", CVAR_ARCHIVE, "Pulse the vitals when low");
  cg_entityBob = cgi.AddCvar("cg_entity_bob", "1", CVAR_ARCHIVE, "Controls the bobbing of items");
  cg_entityRotate = cgi.AddCvar("cg_entity_rotate", "1", CVAR_ARCHIVE, "Controls the rotation of items");
  cg_forceSkin = cgi.AddCvar("cg_force_skin", "", CVAR_ARCHIVE | CVAR_R_MEDIA, "Force all other players to use this model/skin (e.g. \"enforcer/default\").");
  ...
  cg_fovZoom = cgi.AddCvar("cg_fov_zoom", "55", CVAR_ARCHIVE, "Zoomed in field of view");
  cg_fovInterpolate = cgi.AddCvar("cg_fov_interpolate", "1", CVAR_ARCHIVE, "Interpolate between field of view changes (default 1.0).");
  cg_hitSound = cgi.AddCvar("cg_hit_sound", "1", CVAR_ARCHIVE, "If a hit sound is played when damaging an enemy.");
  cg_predict = cgi.AddCvar("cg_predict", "1", 0, "Use client side movement prediction");
  cg_quickJoinMaxPing = cgi.AddCvar("cg_quick_join_max_ping", "200", CVAR_ARCHIVE, "Maximum ping allowed for quick join");
  cg_quickJoinMinClients = cgi.AddCvar("cg_quick_join_min_clients", "1", CVAR_ARCHIVE, "Minimum clients allowed for quick join");
  cg_spritePhysics = cgi.AddCvar("cg_sprite_physics", "1", CVAR_ARCHIVE, "Whether to enable sprite physics or not.");
  ...
  cg_cameraMode = cgi.AddCvar("cg_camera_mode", "0", 0,
                               "How the spectator and demo playback camera frames its subject: "
                               "0 first person, 1 third person, 2 follow.");

  cg_thirdPerson = cgi.AddCvar("cg_third_person", "0", CVAR_ARCHIVE | CVAR_DEVELOPER, "Activate third person perspective.");
  cg_thirdPersonX = cgi.AddCvar("cg_third_person_x", "-200", CVAR_ARCHIVE, "The x offset for third person perspective.");
  cg_thirdPersonY = cgi.AddCvar("cg_third_person_y", "0", CVAR_ARCHIVE, "The y offset for third person perspective.");
  cg_thirdPersonZ = cgi.AddCvar("cg_third_person_z", "40", CVAR_ARCHIVE, "The z offset for third person perspective.");
  cg_thirdPersonPitch = cgi.AddCvar("cg_third_person_pitch", "0", CVAR_ARCHIVE, "The pitch offset for third person perspective.");
  cg_thirdPersonYaw = cgi.AddCvar("cg_third_person_yaw", "0", CVAR_ARCHIVE, "The yaw offset for third person perspective.");

  cg_autoSwitch = cgi.AddCvar("auto_switch", "1", CVAR_USER_INFO | CVAR_ARCHIVE, "The weapon auto-switch mode. 0 disables, 1 switches from Blaster only, 2 always switches, 3 switches to new weapons.");
...
#if defined(G_HOOK)
  cg_hookStyle = cgi.AddCvar("hook_style", "pull", CVAR_USER_INFO | CVAR_ARCHIVE, "Your preferred hook style. Can be either \"pull\", \"swing_manual\", or \"swing_auto\".");
#endif
