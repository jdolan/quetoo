...
static void Cl_DrawNetGraph(void) {
  ...
  x = r_device.w - NET_GRAPH_WIDTH;
  y = r_device.h - NET_GRAPH_Y - netgraph_height;
...
static void Cl_DrawSampleCounter(char *buffer, size_t buffer_length, const char *title, uint16_t *samples) {
  ...
  q_snprintf(buffer, buffer_length, "%3u%s (^1%3u ^2%3u^7)", samples[cl.sample_index], title, min, max);
}
...
static void Cl_DrawCounters(void) {
  ...
  int32_t x = r_device.w - 7 * cw;
  int32_t y = r_device.h - 4 * ch;
  ...
  if (cl_draw_position->integer) {
    R_Draw2DString(r_device.w - 14 * cw, y - ch, va("%4.0f %4.0f %4.0f",
