...
static void SetMaterialFlags(brush_side_t *side) {
  ...
  if (!strcmp(side->texture, "common/caulk")) {
    side->surface |= SURF_NO_DRAW;
  } else if (!strcmp(side->texture, "common/clip")) {
    side->contents |= CONTENTS_PLAYER_CLIP;
  } else if (!strcmp(side->texture, "common/dust")) {
    side->contents |= CONTENTS_ATMOSPHERIC;
  } else if (!strcmp(side->texture, "common/hint")) {
    side->surface |= SURF_HINT;
  } else if (!strcmp(side->texture, "common/ladder")) {
    side->contents |= CONTENTS_LADDER | CONTENTS_PLAYER_CLIP;
  } else if (!strcmp(side->texture, "common/monsterclip")) {
    side->contents |= CONTENTS_MONSTER_CLIP;
  } else if (!strcmp(side->texture, "common/origin")) {
    side->contents |= CONTENTS_ORIGIN;
  } else if (!strcmp(side->texture, "common/skip")) {
    side->surface |= SURF_SKIP;
  } else if (!strcmp(side->texture, "common/sky")) {
    side->surface |= SURF_SKY;
  } else if (!strcmp(side->texture, "common/trigger")) {
    side->surface |= SURF_NO_DRAW;
  } else if (!strcmp(side->texture, "common/weather")) {
    side->contents |= CONTENTS_ATMOSPHERIC;
  }
...
static entity_t *ParseEntity(parser_t *parser) {
  ...
  if (!strcmp(token, "{")) {
    ...
    if (!strcmp(classname, "func_group") ||
      !strcmp(classname, "misc_dust") ||
      !strcmp(classname, "misc_sprite") ||
      !strcmp(classname, "misc_weather")) {
