...
static inline __attribute__((always_inline))
void parallax_occlusion_mapping(thread const common_vertex_t& vertex0, thread common_fragment_t& fragment0, texture2d_array<float> texture_material, sampler texture_materialSmplr, constant material_block& material, constant uniforms_block& _521)
{
    fragment0.parallax = vertex0.diffusemap;
    bool _1783 = material.parallax == 0.0;
    bool _1790;
    if (!_1783)
    {
        _1790 = fragment0.texture_lod > 2.0;
    }
    else
    {
        _1790 = _1783;
    ...
    if (!_1790)
    {
        _1800 = fragment0.view_dist >= (_521.lighting_distance + 128.0);
    }
    else
    {
        _1800 = _1790;
    ...
    float sweep = length(p / texel) / exp2(fragment0.texture_lod);
    float budget = mix(96.0, 24.0, fragment0.texture_lod_normalized);
    float num_samples = fast::clamp(ceil(sweep * 6.0), 1.0, budget);
...
float sample_shadow_atlas(thread const light_t& light, thread const common_vertex_t& v, thread const common_fragment_t& f, thread const float& atten, depth2d<float> texture_shadow_atlas_0, sampler texture_shadow_atlas_0Smplr, depth2d<float> texture_shadow_atlas_1, sampler texture_shadow_atlas_1Smplr, depth2d<float> texture_shadow_atlas_2, sampler texture_shadow_atlas_2Smplr, depth2d<float> texture_shadow_atlas_3, sampler texture_shadow_atlas_3Smplr, depth2d<float> texture_shadow_atlas_4, sampler texture_shadow_atlas_4Smplr, depth2d<float> texture_shadow_atlas_5, sampler texture_shadow_atlas_5Smplr)
{
    ...
    {
        _984 = (importance > 0.100000001490116119384765625) ? 4 : 2;
    }
    int num_samples = _984;
    float s = f.shadow_sin_cos.x;
    float c = f.shadow_sin_cos.y;
    float shadow = 0.0;
    for (int i = 0; i < num_samples; i++)
    {
        float2 rotated = float2((c * _1063[i].x) - (s * _1063[i].y), (s * _1063[i].x) + (c * _1063[i].y));
...
fragment main0_out main0(main0_in in [[stage_in]], constant uniforms_block& _521 [[buffer(0)]], constant bsp_locals_block& _1624 [[buffer(1)]], constant material_block& material [[buffer(2)]], const device bsp_lights_block& _1587 [[buffer(3)]], const device dynamic_lights_block& _1617 [[buffer(4)]], const device voxel_light_data_block& _572 [[buffer(5)]], const device voxel_light_indices_block& _590 [[buffer(6)]], texture2d_array<float> texture_material [[texture(0)]], depth2d<float> texture_shadow_atlas_0 [[texture(1)]], depth2d<float> texture_shadow_atlas_1 [[texture(2)]], depth2d<float> texture_shadow_atlas_2 [[texture(3)]], depth2d<float> texture_shadow_atlas_3 [[texture(4)]], depth2d<float> texture_shadow_atlas_4 [[texture(5)]], depth2d<float> texture_shadow_atlas_5 [[texture(6)]], texture3d<float> texture_voxel_caustics [[texture(7)]], texture3d<float> texture_voxel_occlusion [[texture(8)]], texturecube<float> texture_sky [[texture(9)]], texture2d<float> texture_stage [[texture(10)]], texture2d<float> texture_stage_next [[texture(11)]], texture2d<float> texture_warp [[texture(12)]], sampler texture_materialSmplr [[sampler(0)]], sampler texture_shadow_atlas_0Smplr [[sampler(1)]], sampler texture_shadow_atlas_1Smplr [[sampler(2)]], sampler texture_shadow_atlas_2Smplr [[sampler(3)]], sampler texture_shadow_atlas_3Smplr [[sampler(4)]], sampler texture_shadow_atlas_4Smplr [[sampler(5)]], sampler texture_shadow_atlas_5Smplr [[sampler(6)]], sampler texture_voxel_causticsSmplr [[sampler(7)]], sampler texture_voxel_occlusionSmplr [[sampler(8)]], sampler texture_skySmplr [[sampler(9)]], sampler texture_stageSmplr [[sampler(10)]], sampler texture_stage_nextSmplr [[sampler(11)]], sampler texture_warpSmplr [[sampler(12)]], float4 gl_FragCoord [[position]])
{
    ...
    float2 _1976;
    _1976.x = texture_material.calculate_clamped_lod(texture_materialSmplr, vertex0.diffusemap);
    _1976.y = texture_material.calculate_unclamped_lod(texture_materialSmplr, vertex0.diffusemap);
    fragment0.texture_lod = _1976.x;
    int num_mipmap_levels = int(texture_material.get_num_mip_levels());
    fragment0.texture_lod_normalized = fast::clamp(fragment0.texture_lod / float(num_mipmap_levels - 1), 0.0, 1.0);
    common_vertex_t param = vertex0;
    common_fragment_t param_1 = fragment0;
    parallax_occlusion_mapping(param, param_1, texture_material, texture_materialSmplr, material, _521);
    fragment0 = param_1;
    if (material.flags == 0)
    {
        ...
        fragment0 = param_4;
        float4 _2028 = out.out_color;
        float3 _2030 = _2028.xyz * (fragment0.ambient + fragment0.diffuse);
        out.out_color.x = _2030.x;
        out.out_color.y = _2030.y;
        out.out_color.z = _2030.z;
        float4 _2039 = out.out_color;
        float3 _2041 = _2039.xyz + fragment0.specular;
        out.out_color.x = _2041.x;
        out.out_color.y = _2041.y;
        out.out_color.z = _2041.z;
    }
    else
    {
        ...
        if ((material.flags & 65536) == 65536)
        {
            ...
            fragment_lighting_lod(param_6, param_7, texture_material, texture_materialSmplr, material, _521, _572, _590, texture_voxel_caustics, texture_voxel_causticsSmplr, texture_voxel_occlusion, texture_voxel_occlusionSmplr, texture_shadow_atlas_0, texture_shadow_atlas_0Smplr, texture_shadow_atlas_1, texture_shadow_atlas_1Smplr, texture_shadow_atlas_2, texture_shadow_atlas_2Smplr, texture_shadow_atlas_3, texture_shadow_atlas_3Smplr, texture_shadow_atlas_4, texture_shadow_atlas_4Smplr, texture_shadow_atlas_5, texture_shadow_atlas_5Smplr, texture_sky, texture_skySmplr, _1587, _1617, _1624);
            fragment0 = param_7;
            float4 _2113 = out.out_color;
            float3 _2115 = _2113.xyz * mix(float3(1.0), fragment0.ambient + fragment0.diffuse, float3(material.lighting));
            out.out_color.x = _2115.x;
            out.out_color.y = _2115.y;
            out.out_color.z = _2115.z;
            float4 _2127 = out.out_color;
            float3 _2129 = _2127.xyz + (fragment0.specular * material.lighting);
            out.out_color.x = _2129.x;
            out.out_color.y = _2129.y;
            out.out_color.z = _2129.z;
        }
        if ((material.flags & 262144) == 262144)
        {
            float4 _2150 = out.out_color;
            float3 _2152 = _2150.xyz + (fragment0.diffuse_sample.xyz * material.emissive);
            out.out_color.x = _2152.x;
            out.out_color.y = _2152.y;
            out.out_color.z = _2152.z;
        }
    }
    return out;
}
