...
bool G_Ai_DropItemLikeNode(g_entity_t *ent) {
	...
	cm_trace_t down = gi.Trace(ent->s.origin, Vec3_Subtract(ent->s.origin, Vec3(0, 0, MAX_WORLD_COORD)), Bounds_Zero(), NULL, CONTENTS_MASK_SOLID);
	vec3_t pos;
	...
	if (src_links) {
		...
		for (guint i = 0; i < src_links->len; i++) {
			...
			// can't see 
			if (gi.Trace(ent->s.origin, link_pos, Bounds_Zero(), NULL, CONTENTS_MASK_SOLID).fraction < 1.0) {
				continue;
			}
