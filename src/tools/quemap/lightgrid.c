...
static int32_t ProjectLightgridLuxel(luxel_t *l, float soffs, float toffs, float uoffs) {
	...
	const float s = lg.stu_bounds.mins.x + padding_s + l->s + .5f + soffs;
	const float t = lg.stu_bounds.mins.y + padding_t + l->t + .5f + toffs;
	const float u = lg.stu_bounds.mins.z + padding_u + l->u + .5f + uoffs;
