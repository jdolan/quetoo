...
/**
 * @brief Writes the fixed-size header, embedded thumbnail, `server_data`, `config_strings`, and
 * baselines. Called once, lazily, from the first `Cl_WriteDemoMessage` call after `record`:
 * baselines are already fully populated by then, since reaching `CL_ACTIVE` (a precondition for
 * `record`) requires having received them at connect time.
 */
static void Cl_WriteDemoHeader(void) {
  ...
  byte *thumbnail = R_PollDemoThumbnail(&thumbnail_size);
  ...
  header->favorite = 0;
  header->thumbnail_length = LittleLong((int32_t) thumbnail_size);
  header->data_offset = LittleLong((int32_t) (sizeof(*header) + thumbnail_size));
  header->duration = 0;
  ...
  if (thumbnail) {
    Fs_Write(cls.demo.file, thumbnail, thumbnail_size, 1);
    Mem_Free(thumbnail);
  }
