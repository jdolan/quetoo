...
//	assert(Vec3_Length(s) > 0.f);
//	assert(Vec3_Length(t) > 0.f);

	const float n_dot_s = Vec3_Dot(s, normal);
	
//	assert(n_dot_s != 0.f); // this one fails all the time, scale by zero and you get a vector of zeroes
...
/* This is the function from Q3MAP2
_Bool CalcTangentVectors(int32_t numVerts, bspDrawVert_t **dv, vec3_t *stv, vec3_t *ttv) {
