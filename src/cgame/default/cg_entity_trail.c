...
static void Cg_InitProjectile(const cl_entity_t *ent, vec3_t *forward, vec3_t *right, vec3_t *up, vec3_t *org, float hand) {
	...
	const cm_trace_t tr = cgi.Trace(start, end, Bounds_Zero(), ent->current.number, CONTENTS_MASK_CLIP_PROJECTILE);
	...
	*org = Vec3_Fmaf(start, 12.f, cgi.view->forward);
	...
	if (cgi.Trace(*org, *org, Bounds_Zero(), ent->current.number, CONTENTS_MASK_CLIP_PROJECTILE).start_solid) {
		*org = ent->origin;
	}
