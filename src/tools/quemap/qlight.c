...
cm_trace_t Light_Trace(const vec3_t start, const vec3_t end, int32_t head_node, int32_t mask) {
	...
	cm_trace_t trace = Cm_BoxTrace(start, end, Vec3_Zero(), Vec3_Zero(), 0, mask, NULL, NULL);

	if (head_node) {
		cm_trace_t tr = Cm_BoxTrace(start, end, Vec3_Zero(), Vec3_Zero(), head_node, mask, NULL, NULL);
