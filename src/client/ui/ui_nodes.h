...
typedef struct menuNode_s {
	...
	byte textalign;
	boolean_t invis;				/**< true if the node is invisible */
	boolean_t disabled;			/**< true if the node is inactive */
	boolean_t invalidated;		/**< true if we need to update the layout */
	boolean_t ghost;				/**< true if the node is not tangible */
	char* text;
	...
	void* image;
	void* cvar;
	boolean_t state;				/**< is node hovered */
	int mousefx;
	...
	/* zone */
	boolean_t repeat;			/**< repeat action when "click" is held */
...
typedef struct nodeBehaviour_s {
	...
	boolean_t isVirtual;				/**< true, if the node doesn't have any position on the screen */
	boolean_t isFunction;			/**< true, if the node is a function */
	boolean_t isAbstract;			/**< true, if we can't instantiate the behaviour */
	boolean_t isInitialized;			/**< cache if we already have initialized the node behaviour */
	boolean_t focusEnabled;			/**< true if the node can win the focus (should be use when type TAB) */
	...
	void (*mouseWheel)(menuNode_t *node, boolean_t down, int x, int y);	/**< mouse wheel event in the node */
	...
	boolean_t (*keyPressed)(menuNode_t *node, unsigned int key, unsigned short unicode);
...
menuNode_t* MN_CloneNode(const menuNode_t* node, struct menuNode_s *targetMenu, boolean_t recursive);
